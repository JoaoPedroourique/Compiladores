...
# define YYTOKENTYPE
  enum yytokentype
  {
    ...
    YY_PARSE_programa = 301,       /* YY_PARSE_programa  */
    YY_PARSE_decl = 302            /* YY_PARSE_decl  */
  };
...
// Return type when parsing one decl.
typedef struct
{
  int yystatus;
  int yynerrs;
...
// Parse one decl.
yyparse_decl_t yyparse_decl (void);

// Return type when parsing one programa.
typedef struct
{
  int yystatus;
  int yynerrs;
...
// Parse one programa.
yyparse_programa_t yyparse_programa (void);
